...
### functions related to using ava to segment the audio ###
########################################################################################
...
##combine multiple annotations into a single csv and save
#def combine_annotations(source_dir, save_dir):
    #Use combine_dataframes() instead
    #TODO delete this
#	"""
#	combine multiple annotations into a single csv and save
#
...
#
#	"""
#	sources = [source_dir+i for i in os.listdir(source_dir) if i.endswith('.csv') and not 'all' not in i and not i.startswith('.')]
#	combined = []
#	source_files = []
#
#	for i in sources:
...
#		temp = pd.read_csv(i)
#		combined.append(temp)
#		source_files.extend([i.split('/')[-1].split('_annotations.csv')[0]+'.wav']*len(temp))
#
#	all_annotations = pd.concat(combined)
#	all_annotations['source_file'] = source_files
#	all_annotations.to_csv(save_dir+'all_annotations.csv', index=False)
#
#def combine_predictions(source_dir, save_dir):
#	"""
#	combine multiple predictions csvs into a single csv and save
#
#	Parameters
#	----------
#	source_dir (string): the path to the directory containing the csvs to be combined
#
#	save_dir (string): the path to the directory where the combined csv will be saved
#
#	Returns
...
#
#	"""
#	sources = [source_dir+i for i in os.listdir(source_dir) if i.endswith('.csv') and not i.startswith('all') and not i.startswith('.')] #if an aggregate file already exists ignore it
#	
#	combined = []
#	source_files = []
#
...
#		temp = pd.read_csv(i)
#		combined.append(temp)
#		source_files.extend([i.split('/')[-1].split('.csv')[0]+'.wav']*len(temp))
#
#	all_annotations = pd.concat(combined)
#	all_annotations['source_file'] = source_files
#	all_annotations['species'] = [i.split('_')[0] for i in all_annotations['source_file']]
#
#	all_annotations.to_csv(save_dir+'all_predictions.csv', index=False)


#segment the audio without using ava file structure
def get_amplitude_segments(audio_dir, save_dir, seg_params, species = None, thresholds_path=None, intersyll_threshold = None, duration_threshold = None, path_list = None):
    """
    Segment audio files that are organized in the file structure required by ava. Writes a separate csv for non-vocal pups
...
def get_intersyllable_intervals(df, annotation=False):
	...
	#Occasionally one predicted segment with end at exactly the same time as the next one starts for some reason, which causes problems for sorting. Drop these before sorting.
...
#merge vocalizations with an intersyllable interval below a threshold (ms) from each other and drop vocalizations that are shorter than a threshold (ms)
...
#verions of prune_segments that takes a path instead of a dataframe and doesn't reurn anythign (just saves a csv). Useful for testing on annotated recordings.
