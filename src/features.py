...
## functions related to assessing audio quality (clipping and signal to noise)
###############################################################################

#use noise clips generated by get_silence() to calculate signal to noise
#modified from warlber
...
def get_snr_batch(clip_dir, noise_dir, species, algorithm):
	...
	if species != None:
		...
		pups = [i for i in os.listdir(clip_dir) if not i.startswith('.')]
	...
	for pup in tqdm(pups):
		...
		#get the audio
		clip_path = clip_dir+pup
		noise_path = noise_dir+pup.split('_clip')[0]+'_silence-clip_.wav'
	
		fs, noise = wavfile.read(noise_path)
		_, signal = wavfile.read(clip_path)

		if algorithm == 1: #slow (about 30 minutes for 40000 vocs)
			#calculate snr
			signal_amplitude_envelope = np.abs(hilbert(signal))
			noise_amplitdue_envelope = np.abs(hilbert(noise))
			snr = np.mean(signal_amplitude_envelope)/np.mean(noise_amplitdue_envelope)
		...
		elif algorithm == 2: #MUCH faster than algorithm 1 (about 3 minutes for 40000 vocs)
...
def get_clipping(wav, threshold):
