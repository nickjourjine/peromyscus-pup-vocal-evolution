...
def get_noise_clip(pup, audio_dir, seg_csv, save_dir, margin=0, min_dur=2, max_dur=3, units = 's'):
    ...
    Arguments:
        ...
        min_dur (float): the minimum duration in seconds of the desired noise clips
        max_dur (float): the maximum duration in seconds of the desired noise clips
...
def get_noise_floor(noise_dir, thresh, species, spec_params, save_dir, verbose=False, save = False):
    """
    Calculate noise thresholds from the wav files generated by get_noise_clip()

    Arguments:
        noise_dir (string): the path to the directory containing the noise clips generated by get_noise()
        thresh (int): standard deviations above the median spectrogram pixel value of the noise clip above which a signal will no longer be noise
        spec_param (dict): parameters for generating spectrograms of noise clips with these keys: 'nperseg', 'noverlap', 'fs', 'min_freq', 'max_freq'
        verbose (bool): if True, histograms of the spetrogram pixel values with the thresold drawn as a verticle line will be saved to noise_dir
        save (bool): if true, a dataframe of the noise_floors for each noise clip will be saved to noise_dir
...
def sample_vocs(frame, num_to_sample, label_to_sample, random_state):
	...
	label_to_sample (0 or 1): the label to sample (0 corresponds to "USV" cluster, 1 to "cry" cluster)	
	random_state (int): random seed for reproducible sampling
	
	Returns
	-------
	downsampled_df (df): a downsampled dataframe containing the spectrogram images, umap_coordinates, source_files, and hdbscan labels for annotating
...
def from_background(species, pup, bg_labeling_params, bg_clips_dir, df_save_dir, df_save_name, inprogress_dir):
	...
	3. Concatenate those clips that don't contain vocalizations and save them.
    
    Useful for generating training data of nonvocal sound.
	...
	Arguments:
        ...
        bg_clips_dir (str): path to the directory containing background wav clips (within wich there are subdirectories labeled by species 2 letter code)
	...
	Returns:
	   validated_df (df): a dataframe containing the source_files of the background clips that do not contain vocalizations
...
def wav_from_background_annotations(frame, clips_dir, save_dir, save_name):
	"""
	1. Take a dataframe of human labeled background clips and their source files
	2. Load the audio for those source files, trim them if they had been stimmed during annotation, and concatenate it all together
	3. Save that big wav file
...
def wav_from_nonvocal_annotations(frame, clips_dir, save_dir, save_name):
	"""
	1. Take a dataframe of human labeled non_vocal clips from a given cluster and their source files
	2. Load the audio for those source files and concatenate it all together
	3. Save that big wav file
...
def clean_voc_labels(frame, save_dir, voc_save_name, nonvoc_save_name):
	"""
	1. Take a dataframe of human labeled spectrograms 
	2. If hdbscan label = 0, drop everything that does not have a human label of 'whistle'
	3. Else if hdbscan label = 1, drop everything that does not have a human label of 'cry'
	4. In either case also save a df of just nonvocal sounds
	5. Retrun the dataframe and save
...
def concat_annotated_spacer(voc_clips_dir, voc_clips_list, spacer_wav, save_dir, save_name, label, margin):
    """
    1. Interleve vocalizations annotated from umap with a background nonvocal clip to which noise is randomly added.
    2. Generate an annotations file of the vocalizations start and stop times
    3. Optionally include a margin to trim the annotated start and stop time by some amount.
    
    Useful for organizing annotations into audio files in a way that makes training models easier.
    The format of the annotation csv saved is formatted for use by DAS (https://github.com/janclemenslab/das)
    ...
    Arguments:
        ...
        save_dir (str): the path to the place where the big wav file will be stored
        save_name (str): the name of the annotations csv (no file extension, must end in '_annotations') - this determines the name of the wav file 
    ...
    Returns
    -------
        annotation (dataframe): the annotation csv
...
def get_random_durations(num_to_generate,random_seed, min_dur, max_dur):
	...
	Useful for generating background clips to inerleve with vocalization clips.
...
def starts_stops_from_random_durations(durations):
	...
	Intended to be used after get_random_durations()
...
def random_clips_from_background(background_wav_path, save_dir, clip_name_root, voc_clips_dir, random_seed, min_dur, max_dur):
	...
	Inteneded to be used to get background clips for interleving by concat annotated clips
...
def concat_annotated_random(voc_clips_dir,random_bg_clips_dir, save_dir, save_name, label):
	"""
	1. Interleve vocalizations in voc_clips_dir with clips in random_bg_clips_dir generated from wav file annotations_from_background makes
	2. Save the audio
	3. Save an annotation file
    
    NOTE: Does the same thing as concat_annotated_spacer but at the risk of including vocalizations that were missed during annotation. Use concat_annotated_spacer unless you are 100% confident there are no vocalizations in the annotate background.
    ...
    Arguments:
        ...
        label (str): the label for the vocalization to be interleved (either 'cry' or 'USV' )
